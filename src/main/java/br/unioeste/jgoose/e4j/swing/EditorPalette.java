...
/*     */
import br.unioeste.jgoose.util.BPMNElement;
...
/*     */
 ...
 /*     */
    ...
    public void addSwimlane(String name, ImageIcon icon, String style, int width, int height, Object value) /*     */ {
        mxCell cell = new mxCell(value, new mxGeometry(0.0D, 0.0D, width, height), style);
        cell.setVertex(true);
        cell.setConnectable(false);
        cell.setParent((mxCell) cell.getParent());

        addTemplate(name, icon, cell);
    }
    
    public void addGroup(String name, ImageIcon icon, String style, int width, int height, Object value) /*     */ {
        mxCell cell = new mxCell(value, new mxGeometry(0.0D, 0.0D, width, height), style);
    ...
    public void addTemplate(String name, ImageIcon icon, final mxCell cell) /*     */ {
        mxRectangle bounds = (mxGeometry) cell.getGeometry().clone();
        final mxGraphTransferable t = new mxGraphTransferable(new Object[]{cell}, bounds);
        /*     */
 /*     */
 /*     */
        if ((icon != null) && ( /* 195 */(icon.getIconWidth() > 32) || (icon.getIconHeight() > 32))) {
